...
void UART_Init(void){
  ...
  GPIO_PORTC_AFSEL_R |= 0x30; // alt funct on PC4
	GPIO_PORTC_DEN_R |= 0x30;   // digital I/O on PC4
  GPIO_PORTC_PCTL_R = (GPIO_PORTC_PCTL_R&0xFF00FFFF)+0x00220000;
	//GPIO_PORTC_DIR_R &= 0x00;
  GPIO_PORTC_AMSEL_R &= ~0x30; // No analog on PC4
	UART1_IFLS_R = (UART1_IFLS_R & ~0x28) + 0x10;
	UART1_IM_R |= 0x10; //ARM RXRIS

	NVIC_PRI1_R = (NVIC_PRI1_R&0xFF00FFFF)|0x400000;
	NVIC_EN0_R = 1<<6;
	UART1_CTL_R |= 0x0001;  }
...
char UART_InChar(void){
  char letter;
  while(!RxFifo.Get(&letter)){};
  return(letter);
}
// Lab 9
bool UART_InStatus(void){
  // true if input data ready
	if((UART1_DR_R&0x0FF) == 0x02) return true;
  return false;}
...
void UART_InMessage(char *bufPt){
	bufPt[0] = UART_InChar();
	if(bufPt[0] != 0x02) return;
	for(int i = 1; i < 8; i++){
		bufPt[i] = UART_InChar();
		if(bufPt[i] == ETX) break;
	}
	bufPt[7] = UART_InChar();
}
